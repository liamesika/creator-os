...
export default function AgencyTemplatesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch managed creators
      const creatorsRes = await fetch('/api/agency/creators')
      const creatorsData = await creatorsRes.json()
      if (creatorsData.creators) {
        setCreators(creatorsData.creators.map((c: any) => ({
          id: c.id,
          fullName: c.fullName || c.full_name || 'יוצר',
  ...
  // Handle apply template to creator
  const handleApply = async () => {
    if (!showApplyModal || !applyDate || !selectedCreatorId) return
    ...
    try {
      const res = await fetch('/api/templates/apply', {
        ...
        body: JSON.stringify({
          ...
          targetCreatorUserId: selectedCreatorId,
          startDate: applyDate,
        }),
      })
      ...
      if (res.ok && data.success) {
        ...
        toast.success(`נוצרו ${data.summary.eventsCreated + data.summary.tasksCreated + data.summary.goalsCreated} פריטים ל${creator?.fullName || 'יוצר'}`)
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h1 className="text-xl font-bold text-neutral-900">תבניות סוכנות</h1>
                  <p className="text-xs text-neutral-500">החל תבניות ליוצרים שלך</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {/* Creators count */}
              <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-violet-50 text-violet-700 rounded-lg text-sm">
                <Users size={14} />
                <span>{creators.length} יוצרים</span>
              </div>
          ...
          >
            ...
            <p className="text-neutral-500 mb-6">
              {searchQuery || selectedCategory !== 'all'
                ...
                : 'צור תבניות לניהול יעיל של היוצרים שלך'
              }
            </p>
            {!searchQuery && selectedCategory === 'all' && (
              <button
          ...
          >
            ...
            >
              <div className="p-6 border-b border-neutral-100">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold text-neutral-900">החל תבנית ליוצר</h2>
                  <button
                    onClick={() => setShowApplyModal(null)}
              ...
              <div className="p-6 space-y-4">
                <div className="p-4 bg-violet-50 rounded-xl">
                  ...
                  <p className="text-sm text-neutral-600 mt-1">
                    {showApplyModal.items?.length || 0} פריטים יתווספו ליוצר
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Users size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-400" />
                    <select
                      value={selectedCreatorId}
                      onChange={(e) => setSelectedCreatorId(e.target.value)}
                      className="w-full pr-10 pl-4 py-3 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent bg-white appearance-none"
                    >
                ...
                <button
                  onClick={handleApply}
                  disabled={!applyDate || !selectedCreatorId || applying}
...
// Agency Template Card Component
...
function AgencyTemplateCard({ template, index, onApply, onDelete }: AgencyTemplateCardProps) {
    ...
    <PremiumCard delay={index * 0.05} interactive>
      <div className="p-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {template.ownerType === 'agency' && (
            <div className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">
              <Building2 size={10} />
              סוכנות
            </div>
          )}
...
// Create Agency Template Modal (similar to creator but with agency context)
interface CreateAgencyTemplateModalProps {
  onClose: () => void
  onCreated: (template: Template) => void
}

function CreateAgencyTemplateModal({ onClose, onCreated }: CreateAgencyTemplateModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="p-6 border-b border-neutral-100 bg-gradient-to-r from-violet-500/5 to-indigo-500/5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-neutral-900">תבנית סוכנות חדשה</h2>
                <p className="text-sm text-neutral-500">צור תבנית להחלה על יוצרים</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
          ...
          <div>
            ...
            <input
              ...
              placeholder="לדוגמה: קמפיין השקה סטנדרטי"
              className="w-full p-3 border border-neutral-200 rounded-xl focus:ring-2 focus:ring-violet-500 focus:border-transparent"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="תיאור קצר של התבנית לשימוש הצוות..."
              rows={2}
          ...
          <div className="flex items-center justify-between p-4 bg-neutral-50 rounded-xl">
            <div>
              ...
              <p className="text-sm text-neutral-500">יוצרים אחרים יוכלו לראות ולהשתמש</p>
            </div>
            <button
